...
R_10 <- R_10 %>%
  mutate(CORTE_PASADO = ifelse(Caida <= dia_actual, TRUE, FALSE))
...
ruta <- "C:/Users/jjvt9593/OneDrive - Grupo Coomeva/Archivos de Alejandro Ocampo Losada - Bases de gestion asociados-banco/7. Julio"
archivos_con_fechas <- file.info(list.files(path = ruta, full.names = TRUE))
archivos_excel <- archivos_con_fechas[grep("\\.xlsx$", rownames(archivos_con_fechas), ignore.case = TRUE), ]
ultimo_archivo <- rownames(archivos_excel)[which.max(archivos_excel$mtime)]
...
ruta_archivo <- paste0("C:/Users/jjvt9593/OneDrive - Grupo Coomeva/Escritorio/Carpetas/Corrección R-10/Resultado/Seguimientos/R10-", Fecha_hoy,".xlsx")
