...
#NUEVA BASE CAMPAÑA MORA 35 - Se ajustaron los formatos de las fechas de las promesas
...
#CARGA DE LIBRERIAS
  ...
  Mora35 <- dbGetQuery(Connecion_SQL,glue_sql(
    ...
    LEFT JOIN (SELECT *
              FROM (SELECT 
                      ...
                      WHERE g.GES_FECHA_GESTION > ADD_MONTHS(SYSDATE, -2)
                  ) subquery
  ...
  base %>%
    write_xlsx(
      glue(
        "//coomeva.nal/DFSCoomeva/Cartera_Coomeva/CARTERA/1. Estatutaria/Mora_35/2025/2. Febrero/Reporte_Mora35_{v_today}_USO INTERNO.xlsx",
        col_names=TRUE,sheet="Hoja1"))
  
  #--------------------------------------------- ARCHIVO GESTIONES ------------------------------------------------------------
  
  ruta_archivo <- "//coomeva.nal/DFSCoomeva/Cartera_Coomeva/CARTERA/1. Estatutaria/Mora_35/Gestiones/Gestiones_202502.xlsx"
  nombre_hoja <- "Hoja1"
  gestiones_periodo  <- read_excel(ruta_archivo, sheet = nombre_hoja,
    ...
    mutate(CEDULA = as.numeric(CEDULA),
           FECHA_PROMESA = as.Date(FECHA_PROMESA, format = "%d-%m-%Y"), # Convertir a objeto fecha
           FECHA_PROMESA = format(FECHA_PROMESA, "%d/%m/%Y"))
  ...
  lista_archivos <- list.files(path = "//coomeva.nal/DFSCoomeva/Cartera_Coomeva/CARTERA/1. Estatutaria/Mora_35/2025/2. Febrero",
  ...
  for (ids in id_chunks) {
    ...
    query <- glue(
      ...
      WHERE HRCD_FECHA_RECAUDO BETWEEN TO_DATE('01/02/2025', 'DD/MM/YYYY') AND TO_DATE('28/02/2025', 'DD/MM/YYYY') 
  ...
  Facturacion <- Base_Facturacion %>% 
    ...
    filter(HFR_PERIODO_FACT == 202502) %>%  #<------------- cambiar por periodo actual
    mutate(PAGARE = as.numeric(PAGARE)) %>% 
    distinct()
  ...
  ruta_archivo <- "//coomeva.nal/DFSCoomeva/Cartera_Coomeva/CARTERA/1. Estatutaria/Mora_35/2025/Bases_Cruce/Promesas_Mora35_012025.xlsx"
  ...
  Polizas_periodo_anterior <- read_excel(ruta_archivo, sheet = nombre_hoja) %>% 
    ...
    mutate(FECHA_GESTION = format(as.Date(FECHA_GESTION, origin = "1899-12-30"), "%d/%m/%Y"), # Convierte a fecha y formatea
           FECHA_PROMESA = format(as.Date(FECHA_PROMESA, origin = "1899-12-30"), "%d/%m/%Y")) %>% # Convierte a fecha y formatea
  ...
  promesas_polizas %>%
    write_xlsx(
      glue(
        "//coomeva.nal/DFSCoomeva/Cartera_Coomeva/CARTERA/1. Estatutaria/Mora_35/2025/Bases_Cruce/SPP_202502.xlsx",
        col_names=TRUE,sheet="Hoja1"))
  ...
  gestion_polizas %>%
    write_xlsx(
      glue(
        "//coomeva.nal/DFSCoomeva/Cartera_Coomeva/CARTERA/1. Estatutaria/Mora_35/2025/Bases_Cruce/STP_202502.xlsx",
        col_names=TRUE,sheet="Hoja1"))
